@php
  ...
  $formattedSchedule = [];
  foreach ($grouped as $hours => $daysGroup) {
      $dayLabels = array_values($daysGroup);
      $dayCount = count($dayLabels);

      if ($dayCount > 1) {
          $formattedSchedule[] = $dayLabels[0] . ' - ' . $dayLabels[$dayCount - 1] . ': ' . $hours;
      } else {
          $formattedSchedule[] = $dayLabels[0] . ': ' . $hours;
      }
  }
@endphp
      ...
      <div class="flex-grow md:flex-grow-0 md:basis-1/3 mb-6 md:mb-0">
        <a class="flex flex-col items-start">
          ...
          <div class="mt-3 text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
            <p> Adresa: {{ $libraryData['address'] ?? '' }}</p>
            <p>PIB: {{ $libraryData['pib'] ?? '' }}</p>
          </div>
        </a>
      </div>
        ...
        <div>
          <h2 class="mb-6 text-sm font-semibold text-gray-900 uppercase dark:text-white">Radno vreme</h2>
            <ul class="text-gray-500 dark:text-gray-400 font-medium">
              @foreach ($formattedSchedule as $line)
                <li>{{ $line }}</li>
              @endforeach
            </ul>
