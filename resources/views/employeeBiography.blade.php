...
<x-guest-layout>
    ...
    <div class="min-h-[90vh] w-full flex items-start justify-center p-2 px-4 sm:px-6 lg:px-12"
        style="background: var(--primary-bg); color: var(--primary-text);" x-data="{
            editOpen: false,
            helpOpen: false,
            slide: 1,
            total: 2,
        ...
        <div class="w-full max-w-screen-xl mx-auto">
            <div style="background: var(--primary-bg); color: var(--primary-text);">
                <div class="p-2 sm:p-4 lg:p-6" style="color: var(--primary-text);">
                    ...
                    <div class="flex items-center justify-between mb-6">
                        <h1 class="text-3xl sm:text-4xl font-bold" style="color: var(--primary-text);">
                            {{ $employee->translate('position') }}
                        </h1>
                        @auth
                            <div class="flex flex-col items-end">
                                <!-- HELP dugme iznad Edit -->
                                <button @click="helpOpen = true" type="button"
                                    class="flex items-center gap-2 mb-2 px-2 py-1 text-base font-semibold rounded transition
           hover:text-[var(--accent)] focus:outline-none shadow-none bg-transparent border-none text-[var(--secondary-text)]">
                                ...
                                <button @click="editOpen = true"
                                    class="font-semibold px-4 py-2 rounded transition
           ...
           text-white">
                    ...
                    <div class="flex flex-col lg:flex-row items-start gap-8 mb-10"
                        ...
                        <img class="object-cover w-full lg:w-[28rem] h-[36rem] rounded-lg"
                            src="{{ asset($employee->image_path) }}" alt="{{ $employee->name }}"
                            onerror="this.src='{{ asset('/images/default.jpg') }}';"
                            style="background: var(--primary-bg);" />
                        <div class="flex-1">
                            <h2 class="text-2xl sm:text-3xl font-bold mb-4" style="color: var(--primary-text);">
                                {{ $employee->name }}
                            </h2>
        ...
        <div x-show="editOpen" x-transition class="fixed inset-0 flex items-center justify-center z-50"
            ...
            <div x-show="editOpen" x-transition class="rounded-lg shadow-lg max-w-lg w-full p-6 relative"
                ...
                <h2 class="text-xl font-semibold mb-4" style="color: var(--primary-text);">
                ...
                <form method="POST" action="{{ route('employees.updateExtendedBiography', $employee->id) }}">
                    ...
                    @php
                        ...
                        $bioValue = old(
                            $bioField,
                            $employee->extendedBiography ? $employee->extendedBiography->$bioField : '',
                        );
                        $uniValue = old(
                            $uniField,
                            $employee->extendedBiography ? $employee->extendedBiography->$uniField : '',
                        );
                        $expValue = old(
                            $expField,
                            $employee->extendedBiography ? $employee->extendedBiography->$expField : '',
                        );
                        $skillsValue = old(
                            $skillsField,
                            $employee->extendedBiography
                                ? implode(',', $employee->extendedBiography->$skillsField ?? [])
                                : '',
                        );
                    @endphp

                    <label class="block mb-2" style="color: var(--secondary-text);" for="{{ $bioField }}">
                    ...
                    <label class="block mb-2" style="color: var(--secondary-text);" for="{{ $uniField }}">
                        {{ $locale === 'en' ? 'University' : ($locale === 'sr-Cyrl' ? 'Универзитет' : 'Univerzitet') }}
                    </label>
                    <input type="text" name="{{ $uniField }}" id="{{ $uniField }}" required
                        value="{{ $uniValue }}" class="w-full p-2 mb-4 border rounded"
                        style="background: var(--primary-bg); color: var(--primary-text); border-color: var(--secondary-text);" />

                    <label class="block mb-2" style="color: var(--secondary-text);" for="{{ $expField }}">
                        {{ $locale === 'en' ? 'Experience' : ($locale === 'sr-Cyrl' ? 'Искуство' : 'Iskustvo') }}
                    </label>
                    ...
                    <label class="block mb-2" style="color: var(--secondary-text);" for="{{ $skillsField }}">
                        {{ $locale === 'en' ? 'Skills' : ($locale === 'sr-Cyrl' ? 'Вештине' : 'Veštine') }}
                    </label>
                    <input type="text" name="{{ $skillsField }}" id="{{ $skillsField }}"
                        value="{{ $skillsValue }}" class="w-full p-2 mb-4 border rounded"
                        style="background: var(--primary-bg); color: var(--primary-text); border-color: var(--secondary-text);" />
                            ...
                            <div class="flex items-center justify-center w-full" style="min-height: 140px;">
                                ...
                                <div class="flex-1 flex justify-center items-center min-h-[120px] cursor-zoom-in">
                                    <template x-if="slide === 1">
                                        <img @click="enlarged = '/images/extendedEmployee-help1.png'"
                                            src="/images/extendedEmployee-help1.png" alt="Edit Button"
                                    ...
                                    <template x-if="slide === 2">
                                        <img @click="enlarged = '/images/extendedEmployee-help2.png'"
                                            src="/images/extendedEmployee-help2.png" alt="Edit Modal"
                                            class="rounded-xl max-h-36 object-contain bg-transparent transition-all duration-300 shadow hover:scale-105" />
                                    </template>
                                </div>
                                ...
                                <div>
                                    ...
                                    <p>
                                        @switch(App::getLocale())
                                            @case('en')
                                                To edit employee details, click the "Edit" button located at the top right of the
                                                page (see image 1).
                                            @break

                                            @case('sr-Cyrl')
                                                Да бисте измењивали податке, кликните на дугме „Измени“ у горњем десном углу
                                                странице (слика 1).
                                            @break

                                            @default
                                                Da biste izmenili podatke, kliknite na dugme „Izmeni“ u gornjem desnom uglu stranice
                                                (slika 1)
                                                .
                                        @endswitch
                                    </p>
                            ...
                            <template x-if="slide === 2">
                                <div>
                                    ...
                                    <p>
                                        @switch(App::getLocale())
                                            @case('en')
                                                After clicking "Edit", a dialog will open with all fields for the employee's
                                                biography, university, experience, and skills. After making changes, click "Save" to
                                                apply them (see image 2).
                                            @break

                                            @case('sr-Cyrl')
                                                Након клика на „Измени“, отвориће се прозор са свим пољима биографије, универзитета,
                                                искуства и вештина. По завршетку измена, кликните на „Сачувај“ (слика 2).
                                            @break

                                            @default
                                                Nakon klika na „Izmeni“, otvoriće se prozor sa svim poljima biografije,
                                                univerziteta, iskustva i veština. Po završetku izmena kliknite na „Sačuvaj“ (slika
                                                2).
                                        @endswitch
                                    </p>
                                </div>
