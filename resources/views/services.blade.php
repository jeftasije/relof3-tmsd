                        ...
                        <div class="group h-full overflow-hidden rounded-2xl shadow-xl bg-white relative flex-1 flex">
                            <template x-for="(img, idx) in form.images" :key="img">
                                <img
                                    ...
                                    :class="currentImage === idx ? 'opacity-100 z-10' : 'opacity-0 z-0'"
                                    alt=""
        ...
        return {
           ...
           saveEdit() {
                ...
                Promise.all(uploadPromises).then(newImagePaths => {
                    // 1. Prvo izbaci sve base64 iz images (oni su data:image/...)
                    this.form.images = this.form.images.filter(img => !img.startsWith('data:'));
                    // 2. Dodaj sve nove prave putanje
                    this.form.images = [
                        ...this.form.images,
                        ...newImagePaths.filter(Boolean)
                    ];
                    // 3. Sad šalji backendu
                    fetch(updateUrl, {
                        method: 'POST',
                        headers: {
                        ...
                        body: JSON.stringify({
                            ...
                            images: this.form.images,  // sada su ovde SAMO putanje
                        })
                    }).then(res => res.json())
                    .then(data => {
