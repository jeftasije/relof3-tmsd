<x-guest-layout>
    ...
    <x-slot name="header">
        <div class="flex justify-between items-center w-full p-4" id="header">
            ...
            <button id="theme-toggle"
                class="p-2 rounded-full text-gray-900 hover:bg-gray-200 focus:outline-none dark:text-white dark:hover:bg-gray-700">
    ...
    <div style="background: var(--primary-bg) !important; min-height: 90vh;"
        ...
        <div class="w-full max-w-screen-xl mx-auto">
            ...
            <div x-show="$store.modals.addEmployeeOpen" x-transition
                    ...
                    <form method="POST" action="{{ route('employees.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                            <div>
                                ...
                                <textarea name="biography" id="biography" rows="4" class="w-full p-2 mb-4 border border-gray-300 rounded"
                            ...
                            <div>
                                ...
                                <textarea name="experience" id="experience" rows="3" class="w-full p-2 mb-4 border border-gray-300 rounded"
                                    style="background: var(--primary-bg); color: var(--primary-text);"></textarea>
                                ...
                                <textarea name="skills" id="skills" rows="3" class="w-full p-2 mb-4 border border-gray-300 rounded"
                                    style="background: var(--primary-bg); color: var(--primary-text);"></textarea>
                            </div>
                        </div>
            ...
            @auth
                <div x-show="$store.modals.helpOpen" x-transition x-cloak
                    ...
                    tabindex="0" @click.self="$store.modals.closeAll()"
                    @keydown.escape.window="$store.modals.closeAll()">
                    <div x-show="$store.modals.helpOpen" x-transition x-cloak
                        ...
                        @click.stop x-data="{ slide: 1, total: 3, enlarged: false }">
                        ...
                        <div class="flex flex-col flex-1 px-4 py-3 overflow-hidden h-full">
                            <div class="flex flex-col items-center justify-start" style="height: 48%;">
                                <h3 class="text-lg font-bold text-center mb-2" style="color:var(--primary-text)">
                                    {{ App::getLocale() === 'en' ? 'How to use Employee Management' : (App::getLocale() === 'sr-Cyrl' ? 'Како користити управљање запосленима' : 'Kako koristiti upravljanje zaposlenima') }}
                                </h3>
                                <div class="flex items-center justify-center w-full" style="min-height: 170px;">
                                    ...
                                    <div class="flex-1 flex justify-center items-center min-h-[150px] cursor-zoom-in">
                                        <template x-if="slide === 1">
                                            <img @click="enlarged = '/images/employee-help1.png'"
                                                src="/images/employee-help1.png" alt="Edit or Delete Employees"
                                                class="rounded-xl max-h-52 object-contain bg-transparent transition-all duration-300 shadow hover:scale-105" />
                                        </template>
                                        <template x-if="slide === 2">
                                            <img @click="enlarged = '/images/employee-help2.png'"
                                                src="/images/employee-help2.png" alt="Edit Form"
                                                class="rounded-xl max-h-52 object-contain bg-transparent transition-all duration-300 shadow hover:scale-105" />
                                        </template>
                                        <template x-if="slide === 3">
                                            <img @click="enlarged = '/images/employee-help3.png'"
                                                src="/images/employee-help3.png" alt="Add Employee"
                                                class="rounded-xl max-h-52 object-contain bg-transparent transition-all duration-300 shadow hover:scale-105" />
                                        </template>
                                    </div>
                            ...
                            <div class="flex-1 overflow-y-auto px-1 py-1 mt-2"
                                ...
                                <template x-if="slide === 1">
                                    <div>
                                        <h4 class="font-semibold mb-2">
                                            {{ App::getLocale() === 'en' ? 'Editing and Deleting Employees' : (App::getLocale() === 'sr-Cyrl' ? 'Измена и брисање запослених' : 'Izmena i brisanje zaposlenih') }}
                                        </h4>
                                        <p>
                                            @switch(App::getLocale())
                                                @case('en')
                                                    After clicking Edit, you can update fields such as name, position, biography,
                                                    and upload a new photo. Click Save when finished.<br>
                                                    If you wish to discard changes at any moment, click Cancel. A confirmation
                                                    message will appear after successful editing.
                                                @break

                                                @case('sr-Cyrl')
                                                    Након што кликнете на Измени, можете ажурирати поља као што су име, позиција,
                                                    биографија, као и поставити нову фотографију. По завршетку измена кликните на
                                                    дугме Сачувај.<br>
                                                    Ако желите да одустанете од промена у било ком тренутку, кликните на дугме
                                                    Откажи. Након успешне измене добићете потврду.
                                                @break

                                                @default
                                                    Nakon što kliknete na Izmeni, možete ažurirati polja kao što su ime, pozicija,
                                                    biografija i postaviti novu fotografiju. Kada završite, kliknite na Sačuvaj.<br>
                                                    Ako želite da otkažete izmene, kliknite na dugme Otkaži. Nakon uspešne izmene
                                                    dobićete potvrdu.
                                            @endswitch
                                        </p>
                                    </div>
                                ...
                                <template x-if="slide === 2">
                                    <div>
                                        <h4 class="font-semibold mb-2">
                                            {{ App::getLocale() === 'en' ? 'Editing Employee Data' : (App::getLocale() === 'sr-Cyrl' ? 'Уређивање података о запосленом' : 'Uređivanje podataka o zaposlenom') }}
                                        </h4>
                                        <p>
                                            @switch(App::getLocale())
                                                @case('en')
                                ...
                                <template x-if="slide === 3">
                                    <div>
                                        <h4 class="font-semibold mb-2">
                                            {{ App::getLocale() === 'en' ? 'Adding a New Employee' : (App::getLocale() === 'sr-Cyrl' ? 'Додавање новог запосленог' : 'Dodavanje novog zaposlenog') }}
                                        </h4>
                                        <p>
                                            @switch(App::getLocale())
                                                @case('en')
                                                    To add a new employee, click the "Add Employee" button. Fill in the required
                                                    fields such as name, position, biography, and upload a photo.<br>
                                                    Extended fields allow you to add full biography, university, experience, and
                                                    skills.<br>
                                                    After adding an employee, you can view their details by clicking "Show more" on
                                                    their card.
                                                @break

                                                @case('sr-Cyrl')
                                                    Да додате новог запосленог, кликните на дугме "Додај запосленог". Попуните
                                                    обавезна поља као што су име, позиција, биографија и фотографија.<br>
                                                    У проширеним пољима можете додати целу биографију, универзитет, искуство и
                                                    вештине.<br>
                                                    Након додавања запосленог, можете видети детаље кликом на "Прикажи више" на
                                                    картици.
                                                @break

                                                @default
                                                    Da dodate novog zaposlenog, kliknite na dugme "Dodaj zaposlenog". Popunite
                                                    obavezna polja: ime, pozicija, biografija i fotografija.<br>
                                                    U proširenim poljima možete uneti celu biografiju, univerzitet, iskustvo i
                                                    veštine.<br>
                                                    Nakon dodavanja zaposlenog, detalje gledate klikom na "Prikaži više" na kartici
                                                    zaposlenog.
                                            @endswitch
                                        </p>
                                    </div>
            ...
            document.body.addEventListener('click', e => {
                ...
                fetch(link.href, {
                    ...
                    .then(html => {
                        ...
                        const wrapper = doc.getElementById('employees-wrapper');
                        document.getElementById('employees-wrapper').innerHTML = wrapper.innerHTML;

                        if (window.Alpine && Alpine.initTree) {
                            Alpine.initTree(document.getElementById('employees-wrapper'));
                        }
