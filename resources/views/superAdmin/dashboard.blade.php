        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div x-data="{ darkMode: localStorage.getItem('darkMode') === 'true' }" class="text-gray-900 dark:text-gray-100 transition-colors duration-300" style="font-family: 'Roboto', sans-serif;">
                ...
                <section class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-12 animate-fade-in">
                    <div class="glass p-6 rounded-2xl shadow-xl">
                        <h3 class="text-sm font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide mb-4">{{ App::getLocale() === 'en' ? 'Recent Contacts' : (App::getLocale() === 'sr-Cyrl' ? 'Скорашње жалбе' : 'Skorašnje žalbe') }}</h3>
                        <ul class="space-y-3">
                            <li class="text-sm text-gray-700 dark:text-gray-300">{{ App::getLocale() === 'en' ? '"Website too slow" - Anonymous' : (App::getLocale() === 'sr-Cyrl' ? '"Веома спор сајт" - Анонимни корисник' : '"Veoma spor sajt" - Anonimni korisnik') }} <span class="text-gray-500 dark:text-gray-400 text-xs">{{ App::getLocale() === 'en' ? '(2 days ago)' : (App::getLocale() === 'sr-Cyrl' ? '(пре 2 дана)' : '(pre 2 dana)') }}</span></li>
                            <li class="text-sm text-gray-700 dark:text-gray-300">{{ App::getLocale() === 'en' ? '"Document not found" - Jovan M.' : (App::getLocale() === 'sr-Cyrl' ? '"Документ није пронађен" - Јован М.' : '"Dokument nije pronađen" - Jovan M.') }} <span class="text-gray-500 dark:text-gray-400 text-xs">{{ App::getLocale() === 'en' ? '(3 days ago)' : (App::getLocale() === 'sr-Cyrl' ? '(пре 3 дана)' : '(pre 3 dana)') }}</span></li>
                            <li class="text-sm text-gray-700 dark:text-gray-300">{{ App::getLocale() === 'en' ? '"Need more information" - Milica K.' : (App::getLocale() === 'sr-Cyrl' ? '"Потребно више информација" - Милица К.' : '"Potrebno više informacija" - Milica K.') }} <span class="text-gray-500 dark:text-gray-400 text-xs">{{ App::getLocale() === 'en' ? '(4 days ago)' : (App::getLocale() === 'sr-Cyrl' ? '(пре 4 дана)' : '(pre 4 dana)') }}</span></li>
                        </ul>
                    ...
                    <div class="glass p-6 rounded-2xl shadow-xl">
                        <h3 class="text-sm font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide mb-4">{{ App::getLocale() === 'en' ? 'Testimonials' : (App::getLocale() === 'sr-Cyrl' ? 'Похвале' : 'Pohvale') }}</h3>
                        <ul class="space-y-3">
                            <li class="text-sm text-gray-700 dark:text-gray-300">{{ App::getLocale() === 'en' ? '"Great service!" - Marija S.' : (App::getLocale() === 'sr-Cyrl' ? '"Одлична услуга!" - Марија С.' : '"Odlična usluga!" - Marija S.') }}</li>
                            <li class="text-sm text-gray-700 dark:text-gray-300">{{ App::getLocale() === 'en' ? '"Very transparent." - Nikola T.' : (App::getLocale() === 'sr-Cyrl' ? '"Веома транспарентно." - Никола Т.' : '"Veoma transparentno." - Nikola T.') }}</li>
                            <li class="text-sm text-gray-700 dark:text-gray-300">{{ App::getLocale() === 'en' ? '"Easy to navigate." - Stefan P.' : (App::getLocale() === 'sr-Cyrl' ? '"Лако за навигацију." - Стефан П.' : '"Lako za navigaciju." - Stefan P.') }}</li>
                        </ul>
                        <a href="#" class="text-blue-400 text-primary text-sm font-medium hover:underline mt-4 inline-block">{{ App::getLocale() === 'en' ? 'View All' : (App::getLocale() === 'sr-Cyrl' ? 'Погледај све' : 'Pogledaj sve') }}</a>
                    </div>
                </section>
