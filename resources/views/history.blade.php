<x-guest-layout>
    ...
    <div class="max-w-4xl mx-auto py-10 px-6 text-gray-900 dark:text-white">
        ...
        <form action="{{ route('history.update') }}" method="POST" id="historyForm" class="space-y-4">
            ...
            <textarea name="content" id="contentEdit" rows="15" style="text-align: center;"
                class="w-full p-4 border rounded shadow-sm focus:ring focus:outline-non  bg-[color-mix(in_srgb,_var(--primary-bg)_95%,_black_5%)] dark:bg-[color-mix(in_srgb,_var(--primary-bg)_80%,_black_20%)]">{{ old('value', __('history.content')) }}</textarea>

            <div id="editButtons" class="flex justify-end gap-4 hidden">
                <button type="button" id="cancelBtn"
                    class="py-2 px-4 rounded bg-gray-400 hover:bg-gray-500">
                    @switch(App::getLocale())
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        <div class="p-6 text-center">
                            ...
                            <button id="confirmSubmitBtn" type="button"
                                class="bg-[var(--accent)] hover:bg-[color-mix(in_srgb,_var(--accent)_80%,_black_20%)] focus:ring-4 focus:outline-none font-medium rounded text-sm px-5 py-2.5 text-center mr-2">
                                @switch(App::getLocale())
                                @case('en') Save @break
                            ...
                            <button data-modal-hide="submitModal" type="button"
                                class="bg-gray-400 hover:bg-gray-500 focus:ring-4 focus:outline-none rounded text-sm font-medium px-5 py-2.5">
                                @switch(App::getLocale())
                                @case('en') Cancel @break
                                @case('sr-Cyrl') Откажи @break
                                @default Otkaži
    ...
    <div
        ...
        <div class="bg-[var(--primary-bg)] rounded-lg shadow-lg w-full max-w-md p-6 relative">
            <button data-modal-hide="helpModal" class="absolute top-2 right-2 text-gray-500 hover:text-red-500 text-2xl font-bold">
                &times;
            </button>
            <h2 class="text-xl font-bold mb-4 text-center text-[var(--primary-text)]">
                {{ App::getLocale() === 'en' ? 'Help' : (App::getLocale() === 'sr-Cyrl' ? 'Помоћ' : 'Pomoć') }}
            </h2>
            <p class="space-y-2 text-sm text-center text-[var(--secondary-text)]">
                {!! App::getLocale() === 'en'
                ? '
